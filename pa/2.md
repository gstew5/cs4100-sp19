...
3. You must use one of the following languages to implement `vm`: Rust, C++, Python, OCaml. In order for us to support submissions in multiple languages, we ask that *all* submissions contain at least the following three files:
   
   ```
   README               <-- A short text document describing your project's architecture
   Makefile             <-- The Makefile used to build your project
   main.<rs|c|py|ml>    <-- Your project's root file
   ```
   
   NOTES:
   * Typing `make` in the directory in which your files are unpacked should build an executable `vm`, in the same directory as `Makefile`. We will build your project on `prime.cs.ohio.edu` (we suggest you test there).
